...
#l.time_stamp from tweets t INNER JOIN locations l ON t.id = l.id where l.time_stamp >= (NOW() - INTERVAL 16 MONTH) 
#and t.text like '%flood%' or'%wales%' or '%dyfi%' or '%japaneseknotweed%' or '%olympus%' or '%olympos%' and t.location like '%wales%' or 
#'%olympus%' or '%greece%' or '%olympos%'");

$query = mysqli_query($con, "SELECT t.location, t.id, t.text as tweet, t.image_url, t.lon, t.lat, t.time_stamp as time, 
l.time_stamp from tweets t INNER JOIN locations l ON t.id = l.id where l.time_stamp >= (NOW() - INTERVAL 1 MONTH) 
and t.text like ('%flood%' or'%cobweb%' or '%dyfi%' or '%japaneseknotweed%' or '%knotweed%' or '%olympus%' or '%olympos%')");
